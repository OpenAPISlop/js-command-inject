...
Plugins.register({
  ...
  title: 'Chat',
  mount(root, { esc, E }) {
    // ---------- Config (inline so this stays self-contained) ----------
    const CHAT_API = (globalThis.NOVA_CHAT_API || 'https://BillyGPT.com/chat.php');
    const USERNAME_OVERRIDE = (globalThis.NOVA_USERNAME || '');

    // ---------- UI ----------
    const wrap = E('div', { className: 'grid mc-wrap' });

    const chat = E('div', { className: 'card mc-card' });
    chat.innerHTML = `
      <b>Mini Chat</b>
      ...
      <div id="mc-log"></div>
      <div class="mc-resize" id="mc-resize" title="Drag to resize"></div>
    ...
    // ---------- Identity ----------
    const ADJ=['Dank','Epic','Spicy','Salty','Radical','Major','Classic','Vintage','OG','Retro'];
    const NNS=['Cat','Doggo','Llama','Hamster','Turtle','Shibe','Pigeon','Duck'];
    const rand=a=>a[(Math.random()*a.length)|0];
    const myName=(USERNAME_OVERRIDE&&String(USERNAME_OVERRIDE).trim())||(`${rand(ADJ)}${rand(NNS)}${((Math.random()*899)|0)+100}`);
    chat.querySelector('#mc-you').textContent=`You are: ${myName}`;

    // ---------- Color ----------
    const clrEl=chat.querySelector('#mc-color'), clrSave=chat.querySelector('#mc-color-save');
    const keyColor=n=>`nova_user_color_${n}`;
    const getColor=()=>localStorage.getItem(keyColor(myName))||'#a5cfff';
    const setColor=c=>localStorage.setItem(keyColor(myName),c);
    clrEl.value=getColor(); 
    clrSave.onclick=()=>setColor(clrEl.value);
    chat.querySelector('#mc-sw').addEventListener('click', (e)=>{
      const b=e.target.closest('button[data-c]'); if(!b) return;
      clrEl.value=b.dataset.c; setColor(b.dataset.c);
    });

    // ---------- Chat Log ----------
    const log=chat.querySelector('#mc-log');
    const atBottom=()=>Math.abs((log.scrollTop+log.clientHeight)-log.scrollHeight)<6;
    const scrollBottom=()=>{log.scrollTop=log.scrollHeight;};
    const rowHTML=m=>`<div class="mc-row"><b style="color:${esc(getColor())}">${esc(m.name)}</b> <small class="muted">${new Date(m.ts||Date.now()).toLocaleTimeString()}</small><br><span class="mc-txt">${esc(m.text)}</span></div>`;

    // Make log region resizable via CSS resize handle (height controlled by inline style var)
    (function(){
      const res = chat.querySelector('#mc-resize');
      const logBox = log;
      if (!logBox) return;
      // initialize default height variable if not set
      chat.style.setProperty('--mc-log-h', chat.style.getPropertyValue('--mc-log-h') || '180px');
      let dragging=false, startY=0, startH=0;
      const onStart=(e)=>{ dragging=true; startY=(e.touches?e.touches[0].clientY:e.clientY); startH=parseFloat(getComputedStyle(chat).getPropertyValue('--mc-log-h')); e.preventDefault(); };
      const onMove=(e)=>{ if(!dragging) return; const y=(e.touches?e.touches[0].clientY:e.clientY); const nh=Math.max(50, startH + (startY - y)); chat.style.setProperty('--mc-log-h', nh+'px'); };
      const onEnd=()=>{ dragging=false; };
      res.addEventListener('mousedown',onStart); window.addEventListener('mousemove',onMove); window.addEventListener('mouseup',onEnd);
      res.addEventListener('touchstart',onStart,{passive:false}); window.addEventListener('touchmove',onMove,{passive:false}); window.addEventListener('touchend',onEnd);
      // give the log its resizable style
      logBox.style.height = 'var(--mc-log-h, 180px)';
      logBox.style.minHeight = '50px';
      logBox.style.maxHeight = '10vh';
      logBox.style.overflow = 'auto';
      logBox.style.border = '1px solid #1a2942';
      logBox.style.borderRadius = '10px';
      logBox.style.padding = '.65rem';
      logBox.style.background = '#0b1220';
      logBox.style.marginTop = '1rem';
    ...
    async function poll(){
      try{
        const r=await fetch(CHAT_API+(since?('?since='+since):'')); 
        const j=await r.json();
        if(Array.isArray(j)&&j.length){
          const keep=atBottom();
          log.insertAdjacentHTML('beforeend', j.map(rowHTML).join(''));
          since=Math.max(since, ...j.map(x=>x.ts));
    ...
    // ---------- Send ----------
    const msg=chat.querySelector('#mc-msg'), send=chat.querySelector('#mc-send');
    const sendMsg=async()=>{
      ...
      try{
        await fetch(CHAT_API,{
          ...
          body:JSON.stringify({name:myName,text:t})
        });
        setTimeout(poll,100);
      }catch(_){}
    };
