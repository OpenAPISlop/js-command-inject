...
Plugins.register({
  id: 'pluginlab',
  title: 'Plugin Lab',
  async mount(root, hostCtx = {}) {
    // Scoped UI (Shadow DOM)
    const { root: S, E, esc } = novaScopedMount(root);

    // Fallbacks
    const Services = hostCtx.Services || { httpJSON: (u, opt) => (window.fetch || fetch)(u, opt).then(r => r.json()) };
    const on = hostCtx.on || ((el, ev, fn, opt) => { if (!el) return ()=>{}; el.addEventListener(ev, fn, opt); return ()=>el.removeEventListener(ev, fn, opt); });
    ...
    cardEditor.innerHTML = `
      ...
      <div class="muted" style="margin:.35rem 0">
        Paste a plugin that calls <code>Plugins.register({...})</code>.
        Lab injects <code>E</code>, <code>esc</code>, <code>Services</code>, <code>on</code>, <code>fetch</code>, <code>document</code>.
      </div>
      <textarea id="pl-code" class="inp" style="min-height:220px;font-family:ui-monospace,Menlo,Consolas,monospace;"></textarea>
      <div class="row" style="margin-top:.5rem">
        <button id="pl-save" class="run">Save & Run</button>
        <button id="pl-preview-fs" class="run">Preview Full-Screen</button>
        <button id="pl-unmount" class="run">Unmount</button>
        <button id="pl-clear" class="run">Clear Editor</button>
      </div>
      <div id="pl-status" class="muted" style="margin-top:.35rem">Ready.</div>
    `;

    const cardPreview = E('div', { className:'card' });
    cardPreview.innerHTML = `
      <b>Preview</b>
      <div id="pl-mounted-meta" class="muted" style="margin:.35rem 0">No plugin mounted.</div>
      <div id="pl-preview-host" style="border:1px solid #1a2942;border-radius:12px;padding:.65rem;background:#0b1220;min-height:80px"></div>
    `;

    ui.append(cardEditor, cardPreview);
    S.append(ui);

    // ----- refs & state -----
    const LS_KEY = 'nova_pluginlab_snippet';
    const statusEl = cardEditor.querySelector('#pl-status');
    const codeEl   = cardEditor.querySelector('#pl-code');
    const metaEl   = cardPreview.querySelector('#pl-mounted-meta');
    const hostEl   = cardPreview.querySelector('#pl-preview-host');
    const saved    = localStorage.getItem(LS_KEY) || '';
    if (saved) codeEl.value = saved;

    const setStatus = t => { statusEl.textContent = t || '—'; };

    let child = { def:null, cleanup:null, root:null, mode:'card' };

    function clearPreview() {
      try { child.cleanup && child.cleanup(); } catch {}
      child.cleanup = null;
      if (child.root && child.root.remove) { try { child.root.remove(); } catch {} }
      child.root = null; child.def = null; child.mode = 'card';
      hostEl.innerHTML = ''; metaEl.textContent = 'No plugin mounted.';
    }

    // Fullscreen overlay ALSO uses Shadow DOM for strict isolation
    let overlay, overlayShadow, overlayRoot;
    function makeOverlay() {
      ...
      // Scoped overlay
      const scoped = novaScopedMount(overlay, { extraCSS: `
        .bar{position:sticky;top:0;z-index:10;display:flex;gap:.5rem;align-items:center;padding:.5rem;background:rgba(7,12,22,.9);border-bottom:1px solid var(--stroke);backdrop-filter:blur(6px);}
      `});
      overlayShadow = scoped.shadow;
      overlayRoot = scoped.root;

      const bar = document.createElement('div'); bar.className = 'bar';
      const btn = document.createElement('button'); btn.className='run'; btn.textContent='Exit Preview';
      btn.addEventListener('click', exitFullScreenPreview);
      const meta = document.createElement('div'); meta.className='muted'; meta.id='pl-overlay-meta'; meta.style.marginLeft='.5rem';
      bar.append(btn, meta);
    ...
    function exitFullScreenPreview() {
      ...
      if (child.root && child.root.remove) { try { child.root.remove(); } catch {} }
      child.root = null;
      if (overlay) { try { overlay.remove(); } catch {} overlay = null; overlayShadow=null; overlayRoot=null; }
      metaEl.textContent = 'No plugin mounted.'; setStatus('Exited full-screen.');
    ...
    function evalAndCapture(snippet) {
      ...
      const shimPlugins = { register(def){ capturedDef = def; return { ok:true }; } };
      // eslint-disable-next-line no-new-func
      const fn = new Function('Plugins','E','esc','Services','console','on','fetch','document',
        `${snippet}\n;return true;`
      );
      const g = window;
      const prev = { fetch: g.fetch };
      try {
        g.fetch = g.fetch || fetch;
        fn(shimPlugins, E, esc, Services, console, on, g.fetch, document);
      } finally {
        g.fetch = prev.fetch;
      }
      return capturedDef;
    ...
    async function mountChild(def, mode) {
      if (mode === 'fullscreen') {
        makeOverlay();
        document.documentElement.style.overflow = 'hidden';
        document.body.style.overflow = 'hidden';
        // Inner scoped host for plugin content
        const inner = document.createElement('div');
        overlayRoot.append(inner);
        const scoped = novaScopedMount(inner);
        child.root = scoped.root;
        child.mode = 'fullscreen';
        overlayShadow.querySelector('#pl-overlay-meta').textContent =
          `${def.title || '(untitled)'} — id: ${def.id || '(none)'}`;
      } else {
        const inner = document.createElement('div');
        hostEl.append(inner);
        const scoped = novaScopedMount(inner);
      ...
      try {
        maybeCleanup = await def.mount(child.root, { Services, esc, E, on, fetch: window.fetch || fetch });
      } catch (err) {
        const errBox = document.createElement('div');
        errBox.className = 'bad';
        errBox.style.cssText = 'padding:.4rem;border:1px solid #4b1f27;border-radius:8px;background:#1a0f12';
        errBox.textContent = 'Mount error: ' + (err && err.message || String(err));
        child.root.append(errBox);
        throw err;
      }
      child.cleanup = (typeof maybeCleanup === 'function') ? maybeCleanup : null;

      metaEl.innerHTML = `
        <div>
          <span class="muted">Mounted:</span> <b>${esc(def.title || '(untitled)')}</b>
          <span class="muted" style="margin-left:.35rem">id:</span> <code>${esc(def.id || '(none)')}</code>
          <span class="muted" style="margin-left:.35rem">mode:</span> <code>${child.mode}</code>
        </div>`;
    }

    async function runSnippet(snippet, mode='card') {
      setStatus('Evaluating…');
      let def;
      try { def = evalAndCapture(snippet); }
      catch (err) {
        setStatus('Evaluation failed.');
        metaEl.textContent = 'No plugin mounted.';
        hostEl.innerHTML = `<div class="bad" style="padding:.4rem;border:1px solid #4b1f27;border-radius:8px;background:#1a0f12">
          Eval error: ${esc(String(err && err.message || err))}
        </div>`;
        return;
      }
      if (!def || typeof def.mount !== 'function') {
        setStatus('No plugin captured. Ensure it calls Plugins.register({...}).');
        return;
      }
      clearPreview();
      try { await mountChild(def, mode); setStatus(mode==='fullscreen' ? 'Mounted (full-screen).' : 'Mounted.'); } catch {}
    }

    // wire up
    cardEditor.querySelector('#pl-save').addEventListener('click', async () => {
      const snippet = (codeEl.value || '').trim(); if (!snippet) return;
      localStorage.setItem(LS_KEY, snippet);
      await runSnippet(snippet, 'card');
    });
    cardEditor.querySelector('#pl-preview-fs').addEventListener('click', async () => {
      const snippet = (codeEl.value || '').trim(); if (!snippet) return;
      localStorage.setItem(LS_KEY, snippet);
      await runSnippet(snippet, 'fullscreen');
    });
    cardEditor.querySelector('#pl-unmount').addEventListener('click', () => {
      if (child.mode === 'fullscreen') exitFullScreenPreview(); else clearPreview();
      setStatus('Unmounted.');
    });
    cardEditor.querySelector('#pl-clear').addEventListener('click', () => { codeEl.value=''; setStatus('Editor cleared.'); });
    codeEl.addEventListener('keydown', (e) => { if ((e.metaKey||e.ctrlKey) && e.key==='Enter') { e.preventDefault(); cardEditor.querySelector('#pl-save').click(); } });

    if (saved) { setStatus('Found saved snippet. Mounting…'); await runSnippet(saved, 'card'); }

    // cleanup when Lab card removed
    const obs = new MutationObserver(() => {
      if (!S.isConnected) { try { child.cleanup && child.cleanup(); } catch {} obs.disconnect(); if (overlay) exitFullScreenPreview(); }
    });
    obs.observe(document.body, { childList:true, subtree:true });

    setTimeout(() => codeEl.focus(), 50);

    return () => { try { child.cleanup && child.cleanup(); } catch {} if (overlay) exitFullScreenPreview(); };
  }
});
...
Plugins.register({
  ...
  async mount(root, ctx = {}) {
    ...
    cardEditor.innerHTML = `
      <b>Plugin Creator</b>
      <div class="muted" style="margin:.35rem 0">
        Paste a snippet that calls <code>Plugins.register({ id, title, mount })</code>.
      </div>
      <textarea id="pc-code" class="inp" style="min-height:220px;font-family:ui-monospace,Menlo,Consolas,monospace;"></textarea>
      <div class="row" style="margin-top:.5rem">
